...
import {fetchHandcraftItems} from '../services/api.js';
import '../App.css';
import { useEffect, useState } from 'react';

const Handcrafts = () => {
    //state variables
    const [handcraftItems, setHandcraftItems] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);

    //fetching data from database and rendering it on page
    useEffect(() => {
        const getHandcraftItems = async () => {
            try {
                const data = await fetchHandcraftItems();
                setHandcraftItems(data);
                setLoading(false);
            } catch (error) {
    ...
    return (
        <>
            <div style={{ width: '100%', backgroundColor: '#010101', display: 'flex', flexWrap: 'wrap', justifyContent: 'center', padding: '1rem' }}>
                <h1 className="heading" style={{ color: 'white' }}>Handcrafts</h1>
            </div>
